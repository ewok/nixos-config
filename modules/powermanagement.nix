...
with lib;
let
  cfg = config.opt.powermanagement;
...
in
{
  options.opt.powermanagement = {
    enable = mkOption { type = types.bool; };

    # Buggy on P52
    powertop.enable = mkOption { type = types.bool; };
    ...
    suspendHibernate.enable = mkOption { type = types.bool; };
  ...
  config = mkMerge [
    (
      mkIf cfg.enable {
        ...
        #   ];
        # };
        users.users.${cfg.username}.extraGroups = [ "video" ];
      }
    )
