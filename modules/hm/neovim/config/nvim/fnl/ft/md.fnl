...
(lib.reg_ft :markdown #(let [map! lib.map
                         ...
                         (map! [:n] :<leader>wx :<Cmd>ObsidianOpen<CR> md
                               :Links) ; (map! [:n] :<CR> "<Cmd>lua vim.lsp.buf.definition()<CR>" md
                         ;       "Open Link") ; (map! [:v] :<CR> ;       ":'<,'>ZkNewFromTitleSelection { dir = vim.fn.expand('%:p:h') }<CR>"
                         ;       md "Create Link") ; (map! [:n] :<leader>wb :<Cmd>ZkBacklinks<CR> md :Backlinks)
                         ; (map! [:n] :<leader>wl :<Cmd>ZkLinks<CR> md :Links) ; (map! :n :tn "<cmd>MkdnTable 1 1<cr>" md "New Table")
                         ))
