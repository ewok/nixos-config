...
#  if command -q tmux {
    let tmux_cmd = if not ($env.INSIDE_EMACS?|is-empty) { "tmux -L emacs" } else { "tmux" }

    if ( $env.TMUX?|is-empty ) and ( (tty) != "/dev/tty1" ) {
    let sess = (  run-external $tmux_cmd list-sessions) |  lines | where { |it| $it =~ '^[0-9]+'} | where { |it| $it =~ '^((?!attached).)*$'} | split row ':'|if ($in |is-empty) {""} else {$in|first}
      if not ($sess |is-empty) {
        run-external $tmux_cmd attach-session "-t" $sess
        #exit
      } else {
        run-external $tmux_cmd new-session
        #exit
      }
    }
#  }
