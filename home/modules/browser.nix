...
let
  ...
  touchscreen = config.home.gui.touchscreen;
  ...
  chooseBrowser = pkgs.writeShellScriptBin "choose-browser" ''
    CMD=$(echo "vivaldi|firefox|google-chrome-stable" | rofi_run -dmenu -i -l 20 -p "Open url in:" -sep "|")
    $CMD "$1"
  '';
    ...
    programs.firefox = {
      ...
      profiles = {
        options = {
          ...
          } // optionalAttrs (touchscreen.enable) {
